...
|----------|--------------|----------------------------------------------|
| Simple   | O(n²)        | Small stacks / low disorder (< 20%)           |
...
**Authorship: selection logic (disorder calculation + strategy choice) developed by aantela-, integrating the three strategies above.**
...
└── checker_linux                # checker binary provided by 42 to validate the sort
```
...
The repository includes a `checker_linux` binary (provided by 42) that takes the same list of numbers and the instructions generated by `push_swap` via `stdin`, and confirms whether the stack ends up correctly sorted:
...
- **[aantela-](https://github.com/ArturVazOsorio)** (42 Porto):
  ...
  - Norminette compliance (struct extraction such as `t_range`, ternary removal, file splitting).
...
AI (Claude, Anthropic) was used as occasional support for the following tasks, always reviewed and fully understood by both authors before integration:
...
- **Norminette compliance**: suggestions for file splitting, helper extraction, and refactoring to remove ternary operators and reduce the number of parameters per function (e.g. introducing the `t_range` struct).
...
- **Naming review**: suggestions for more descriptive English function names, for better readability during the defense.
